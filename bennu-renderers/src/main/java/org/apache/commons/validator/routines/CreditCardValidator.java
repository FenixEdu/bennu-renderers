 ...
 * </p>
 * <p>
 * By default, all supported card types are allowed. You can specify which cards should pass validation by configuring the
 * validation options. For example,<br/>
 * <code>CreditCardValidator ccv = new CreditCardValidator(CreditCardValidator.AMEX + CreditCardValidator.VISA);</code> configures
 * the validator to only pass American Express and Visa cards. If a card type is not directly supported by this class, you can
 * implement the CreditCardType interface and pass an instance into the <code>addAllowedCardType</code> method.
 * </p>
...
public class CreditCardValidator implements Serializable {
    ...
    public CreditCardValidator(CodeValidator[] creditCardValidators) {
        ...
        for (CodeValidator creditCardValidator : creditCardValidators) {
            cardTypes.add(creditCardValidator);
        }
    }
