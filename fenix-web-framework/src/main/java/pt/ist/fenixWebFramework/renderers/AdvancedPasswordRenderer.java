...
public class AdvancedPasswordRenderer extends InputRenderer {
	...
	return new Layout() {
		...
		script.setScript("$(\"#" + checkBoxId +"\").click( function() { " +
				...
				"if ($(this).attr('checked')) { $(\"#" + passwordInputId +"\").after(\"<input type='text' id=\" + id + \" value=\" + password + \" />\").remove(); }" +
				"else { $(\"#" + passwordInputId +"\").after(\"<input type='password' id=\" + id + \" value=\" + password + \" />\").remove(); }" +		
				"});");
