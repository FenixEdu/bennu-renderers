...
public class ConditionalFormatRenderer extends OutputRenderer {
	 ...
	 * this format would produce something like:
	 * <code>"Jane Doe is 20 years old"</code>
	 * <p>
	 * Any literal text will be present in the output. Slot values can be
	 * including by surrounding the slot name in
	 * <code>${&lt;slot.name&gt;}</code>. Additionally you can indicate how
	 * to format each value. This is specially usefull if you want to
	 * present dates.
	 * 
	 * <pre>
	 ...
	 * <code>"Jane Doe was born in a Sunday"</code> . You can use use the
	 * same syntax that is accepted by #
	...
	public HtmlComponent createComponent(Object object, Class type) {
		...
		try {
			...
			useFormatIfResult = (Boolean) RendererPropertyUtils.getProperty(usedObject, format.getUseFormatIf(),
				false);
		    }
		    if (format.getUseFormatIfNot() != null) {
			useFormatIfNotResult = (Boolean) RendererPropertyUtils.getProperty(usedObject,
				format.getUseFormatIfNot(), false);
		    }
