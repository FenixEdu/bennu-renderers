...
public class CollectionRenderer extends OutputRenderer {
	...
	protected Collection checkForPager(Collection sortedCollection) {
		...
		if (pagedLayout) {
			...
			final Integer page = actualPage == 0 ? Integer.valueOf(1) : Integer
					.valueOf(actualPage);
	...
	public class CollectionTabularLayout extends TabularLayout {
		...
		public HtmlComponent createComponent(Object object, Class type) {
			if (pagedLayout) {
				...
				htmlBlockContainer
						.addChild(super.createComponent(object, type));
				htmlBlockContainer.addChild(linksComponent);
		...
		private List<MetaObject> getMetaObjects(Collection collection) {
			...
			if (contextMetaObject instanceof MetaObjectCollection) {
				...
				for (Object object : collection) {
					for (MetaObject metaObject : multipleMetaObject
							.getAllMetaObjects()) {
				...
				for (Object object : collection) {
					metaObjects.add(MetaObjectFactory.createObject(object,
							schema));
				}
			}
		...
		public HtmlComponent createLayout(Object object, Class type) {
			...
			if (component instanceof HtmlTable) {
				((HtmlTable) component)
						.setRenderCompliantTable(getRenderCompliantTable());
			}
			...
			if (isCheckable() && isSelectAllShown()) {
				...
				if (String.valueOf(getSelectAllLocation()).contains(
						LOCATION_TOP)
						|| String.valueOf(getSelectAllLocation()).contains(
								LOCATION_BOTH)) {
		...
		protected void setExtraComponentOptions(Object object,
				HtmlComponent component, Class type) {
			if (component instanceof HtmlTable) {
				((HtmlTable) component)
						.setRenderCompliantTable(getRenderCompliantTable());
			}
		...
		private void addScripts(HtmlContainer container) {
			...
			String[] scriptNames = new String[] { "jquery.ui.draggable.js",
					"jquery.alerts.js", "alertHandlers.js" };
			for (String script : scriptNames) {
				addSingleScript(container, link, script);
			}
		}

		private void addSingleScript(HtmlContainer container, HtmlLink link,
				String scriptName) {
			link.setUrl("/javaScript/" + scriptName);
			HtmlScript script = new HtmlScript("text/javascript", link
					.calculateUrl(), true);
		...
		private HtmlComponent createInvertSelectionLink() {
			...
			script
					.setScript("function invertSelectionAll(n) {"
							+ "    var allChecked = true;"
			...
			link.setText(RenderUtils
					.getResourceString("renderers.table.selectAll"));
		...
		protected int getNumberOfColumns() {
			if (this.metaObjects.size() > 0) {
				...
				return metaObject.getSlots().size() + getNumberOfLinkColumns()
						+ (isCheckable() ? 1 : 0);
			} else {
				return 0;
			}
		...
		protected HtmlComponent getHeaderComponent(int columnIndex) {
			if (columnIndex == 0 && isCheckable()) {
				return new HtmlText();
			} else if (columnIndex < getNumberOfColumns()
					- getNumberOfLinkColumns()) {
				String slotLabel = getLabel(columnIndex);
				return new HtmlText(slotLabel + getHeaderToolTip(columnIndex),
						false);
			} else {
				return new HtmlText();
			}
		...
		private String getHeaderToolTip(final int columnIndex) {
			MetaSlot metaSlot = this.metaObjects.get(0).getSlots().get(
					columnIndex - (isCheckable() ? 1 : 0));
			String headerToolTip = metaSlot.getProperties() == null ? StringUtils.EMPTY
					: metaSlot.getProperties().getProperty("headerToolTip");
			if (!StringUtils.isEmpty(headerToolTip)) {
				return " <a href=\"#\" class=\"help\">[?]<span>"
						+ RenderUtils.getResourceString(metaSlot.getBundle(),
								headerToolTip) + "</span></a>";
			}
		...
		@Override
		protected void costumizeCell(HtmlTableCell cell, int rowIndex,
				int columnIndex) {
		...
		protected HtmlComponent getComponent(int rowIndex, int columnIndex) {
			...
			if (columnIndex == 0 && isCheckable()) {
				...
				if (getCheckboxValue() != null) {
					...
					try {
						String checkBoxValue = String.valueOf(PropertyUtils
								.getProperty(realObject, getCheckboxValue()));

						boolean checked = false;
						String[] existingValues = getContext().getViewState()
						...
						throw new RuntimeException(
								"could not set check box value by reading property '"
										+ getCheckboxValue() + "' from object "
										+ realObject, e);
					}
				}
				...
				return checkBox;
					...
					- getNumberOfLinkColumns()) {
				return generateObjectComponent(columnIndex
						- (isCheckable() ? 1 : 0), object);
			} else {
				return generateLinkComponent(object, rowIndex, columnIndex
						- (getNumberOfColumns() - getNumberOfLinkColumns()));
			}
		}

		protected HtmlComponent generateObjectComponent(int columnIndex,
				MetaObject object) {
		...
		private void applyConditionalClasses(MetaObject metaObject,
				HtmlComponent component, ColumnCss columnCss) {
			Boolean useIf = ((columnCss.getUseIf() != null) && (Boolean) RendererPropertyUtils
					.getProperty(metaObject.getObject(), columnCss.getUseIf(),
							false));
			Boolean useIfNot = ((columnCss.getUseIfNot() != null) && (Boolean) RendererPropertyUtils
					.getProperty(metaObject.getObject(), columnCss
							.getUseIfNot(), false));
			if (useIf || !useIfNot) {
		...
		protected MetaSlot getSlotUsingName(MetaObject object, int columnIndex) {
			...
			MetaSlot referenceSlot = referenceObject.getSlots()
					.get(columnIndex);
		...
		protected HtmlComponent generateLinkComponent(MetaObject object,
				int rowIndex, int number) {
			if (isGroupLinks() || isRowForLinks()) {
				...
				for (int i = 0; i < getNumberOfLinks(); i++) {
					HtmlComponent component = generateSingleLinkComponent(
							object, rowIndex, i);
					if (component != null) {
						if (includeSeparator) {
							container.addChild(new HtmlText(
									getLinkGroupSeparator(), false));
						}
				...
				HtmlComponent component = generateSingleLinkComponent(object,
						rowIndex, number);
				return component == null ? new HtmlText() : component;
			}
		}

		protected HtmlComponent generateSingleLinkComponent(MetaObject object,
				int rowIndex, int number) {
		...
		protected HtmlComponent wrapPrefixAndSuffix(HtmlComponent component,
				int columnIndex) {
	...
	public HtmlComponent generateLinksComponent(Object object) {
		...
		if (actualPage == 1) {
			...
			linkPrev.setText("<");
			linkPrev.setUrl(getPagingAction() + "&amp;actualPage="
					+ (getActualPage() - 1));
		...
		for (int i = start; i <= end; i++) {
			...
			linkPages.setUrl(getPagingAction() + "&amp;actualPage="
					+ String.valueOf(i));
		...
		if (actualPage == numberOfPages) {
			...
			linkNext.setText(">");
			linkNext.setUrl(getPagingAction() + "&amp;actualPage="
					+ (getActualPage() + 1));
			container.addChild(linkNext);

			HtmlLink linkForth = new HtmlLink();
			linkForth.setText(">>");
			linkForth.setUrl(getPagingAction() + "&amp;actualPage="
					+ numberOfPages);
	...
	public static class TableLink implements Comparable<TableLink> {
		...
		public HtmlComponent generateLink(Object object) {
			if (getVisibleIf() != null) {
				try {
					Boolean visible = (Boolean) RendererPropertyUtils
							.getProperty(object, this.getVisibleIf(), false);
			...
			if (getVisibleIfNot() != null) {
				try {
					Boolean notVisible = (Boolean) RendererPropertyUtils
							.getProperty(object, getVisibleIfNot(), false);
			...
			if (getCustom() != null) {
				return new HtmlText(RenderUtils.getFormattedProperties(
						getCustom(), object), false);
			} else {
				HtmlLink link = new HtmlLink();
				...
				if (getLinkFormat() != null) {
					link.setUrl(RenderUtils.getFormattedProperties(
							getLinkFormat(), object));
				} else {
					link.setUrl(getLink());
				...
				if (getConfirmationKey() != null) {
					...
					final String confirmationMessage = getConfirmationBundle() != null ? RenderUtils
							.getResourceString(getConfirmationBundle(),
									getConfirmationKey())
							: RenderUtils
									.getResourceString(getConfirmationKey());

					final String confirmationTitle = getConfirmationBundle() != null ? RenderUtils
							.getResourceString(getConfirmationBundle(),
									getConfirmationTitleKey())
							: RenderUtils
									.getResourceString(getConfirmationTitleKey());

					HtmlInlineContainer container = new HtmlInlineContainer();
					container.addChild(link);
					String scriptText = "linkConfirmationHook('"
							+ link.getId()
							+ "','"
		...
		public String getLinkText(TableLink tableLink, Object object) {
			...
			return getCounter() != null ? text
					+ RenderUtils.getFormattedProperties(getCounter(), object)
					: text;
		}

		protected void setLinkParameters(Object object, HtmlLink link,
				TableLink tableLink) {
