 ...
 * object.
 * <p>
 * For example, suppose you have the objects <code>Graphic</code>,
 * <code>Rectangle</code>, <code>Square</code>, and <code>Circle</code>.
 * Rectangle and Circle are subclasses of Graphic and Square is a subclass of
 * Rectangle. If you want to present the type of each object then you have to
 * make a choice based on each object's type (assuming they don't have a
 * suitable method to return that text).
 * <p>
 * In this cases, this renderer can be used to present each object's class and
 * create a label from it. The default label format is
...
public class ClassAsLabelRenderer extends OutputRenderer {
     ...
     * <code>"label.${superclass.simpleName}.${simpleName}"</code> would produce
     * the label <code>"label.Rectangle.Square"</code> when applied to the class
     * of a <code>Square</code> object.
     * 
     * @property
     */
